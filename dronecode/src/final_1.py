...
uavstring = ''
setpoint_topic_select=0 #0: PoseStamped, 1 :PositionTarget
using_serial = False
...
def rx_pkt_parser(rx_str):
    rx_pkt_dict = {"cmd_area":[],"mission_start":[],"RTL":[],"mission":[],"beacon":[],"inspection_site":[]}
    ...
    beacon_pkt_list = []
    inspection_site_pkt_index = obtain_index_list(rx_str,binascii.unhexlify('ffaa'))
    inspection_site_pkt_list = []
    ...
    if (len(inspection_site_pkt_index) > 0):
        for i in inspection_site_pkt_index:
            temp=[]
            temp = list(struct.unpack('=BBHB',rx_str[i:i+struct.calcsize('=BBHB')]))
            inspection_site_pkt_list.append(temp)
        rx_pkt_dict["inspection_site"] = inspection_site_pkt_list        
    return rx_pkt_dict
...
class main_class:
    def __init__(self):
        ...
        self.local_id = 1
        while not self.beacon_tx_flag:
            time.sleep(0.9)
            rospy.loginfo("waiting for beacon tx flag to set")
            
        self.wait_function(60)
        ...
        try:
            if len(self.node_data_list)%self.total_inspection_sites == 0 and len(self.node_data_list)!=1 :
        ...
        rospy.loginfo('Area details...:{}'.format(self.inspection_site_dict))
        ...
        while not flag1:
            if self.cluster_number in self.inspection_site_dict.keys():                                             
                self.area_length=self.inspection_site_dict[self.cluster_number][0]
                self.area_breadth=self.inspection_site_dict[self.cluster_number][1]
                self.area_centroid=self.inspection_site_dict[self.cluster_number][2]                                    
                self.current_mission=self.inspection_site_dict[self.cluster_number][3]
                self.ack1_pkt_TX_flag=True
                self.cmd_area_pkt_rxd=True
                flag1=True
        rospy.loginfo('Selected Area details...:{},{},{},{}'.format(self.area_length,self.area_breadth,self.area_centroid,self.current_mission))    
                ...
                #Transmit packets
                    if self.beacon_tx_flag:
                        ...
                        for _ in range(self.tx_retries):
                            ...
                            self.sock.sendto(pkt4_TX_str, (MCAST_GRP , MCAST_PORT))
                    ...
                    if self.mission_pkt_TX_flag:
                        ...
                        for _ in range(self.tx_retries):
                            ...
                            self.sock.sendto(pkt1_TX_str, (MCAST_GRP , MCAST_PORT))
                    ...
                    if self.ack1_pkt_TX_flag:
                        ...
                        for _ in range(self.tx_retries):
                            ...
                            self.sock.sendto(pkt2_TX_str, (MCAST_GRP , MCAST_PORT))
                    ...
                    if self.ack2_pkt_TX_flag:
                        ...
                        for _ in range(self.tx_retries):
                            ...
                            self.sock.sendto(pkt3_TX_str, (MCAST_GRP , MCAST_PORT))
                        rospy.loginfo('ACK sent')
                        self.ack2_pkt_TX_flag = False
    ...
    def com_func_RX(self):	   #Receive packets
        ...
        while (not rospy.is_shutdown() and self.com_thread_flag):
            if using_serial:
                if self.serial_port_status==True:			
                    ...
                    if self.data_len:
                        ...
                        if len(self.rx_pkt_dict["inspection_site"]):
                            for i in range(len(self.rx_pkt_dict["inspection_site"])):
                                self.total_inspection_sites=self.rx_pkt_dict["inspection_site"][i][2]
                                self.beacon_tx_flag=True
                                
                                
                        if len(self.rx_pkt_dict["cmd_area"])>0:	
                            for i in range(len(self.rx_pkt_dict["cmd_area"])):
                                self.inspection_site_dict={self.rx_pkt_dict["cmd_area"][i][7]:[self.rx_pkt_dict["cmd_area"][i][2],self.rx_pkt_dict["cmd_area"][i][3],[(self.rx_pkt_dict["cmd_area"][i][4])/1E7,(self.rx_pkt_dict["cmd_area"][i][5])/1E7 ,(self.rx_pkt_dict["cmd_area"][i][6])/1E3 ],self.rx_pkt_dict["cmd_area"][i][9]]}
            ...
            if not using_serial:
                if self.socket_status == True:
                    self.rx_str = self.sock.recv(10240)                    
                    self.rx_pkt_dict = rx_pkt_parser(self.rx_str)
                    if len(self.rx_pkt_dict["inspection_site"]):
                        for i in range(len(self.rx_pkt_dict["inspection_site"])):
                            self.total_inspection_sites=self.rx_pkt_dict["inspection_site"][i][2]
                    ...
                    if len(self.rx_pkt_dict["cmd_area"])>0:
                    	rospy.loginfo('PKT rxd {}'.format(self.rx_pkt_dict))
                        for i in range(len(self.rx_pkt_dict["cmd_area"])):
                            self.inspection_site_dict[self.rx_pkt_dict["cmd_area"][i][7]]=[self.rx_pkt_dict["cmd_area"][i][2],self.rx_pkt_dict["cmd_area"][i][3],[(self.rx_pkt_dict["cmd_area"][i][4])/1E7,(self.rx_pkt_dict["cmd_area"][i][5])/1E7 ,(self.rx_pkt_dict["cmd_area"][i][6])/1E3 ],self.rx_pkt_dict["cmd_area"][i][9]]
                            rospy.loginfo('self.inspection_site_dict : {}'.format(self.inspection_site_dict))
    ...
    def main(self):
        ...
        position=[[0,0,15,90],[0,15,15,90],[15,15,15,90],[0,0,15,90]]
        ...
        if self.cnt.current_state.mode=='OFFBOARD' and  self.current_mission==2:
            ...
            #self.offset_xyz=[0,0,0]
            self.reach_position(self.list_generated[0][0] + self.offset_xyz[1], self.list_generated[0][1] + self.offset_xyz[0], self.area_centroid[2], 0,120)
            ...
            while not self.RTL_pkt_rxd:		
                if self.local_id==1:
                    for i in range(1,len(self.list_generated)) :
                        if not self.RTL_pkt_rxd:
                            self.reach_position(self.list_generated[i][0] + self.offset_xyz[1],self.list_generated[i][1] + self.offset_xyz[0],self.area_centroid[2],0,30)
        ...
        if self.current_mission==3:
            if self.local_id==1 and self.cnt.current_state.mode=='OFFBOARD':
                ...
                if self.formation_sync_list[self.local_id-2]==1:
                    ...
                    rate=rospy.Rate(5)
                    while not (self.cnt.altitude.local-self.takeoff_alt)>10:
                        rospy.loginfo('Current Altitude : {}'.format(self.cnt.altitude.local))
                        rate.sleep()
                    vehicle_1_id = self.cluster_mav_sys_list[0]
                    lat_long_val = self.data_dict[vehicle_1_id]
                    self.offset_xyz=lla2ned(lat_long_val[0],lat_long_val[1],lat_long_val[2],self.home_lla[0],self.home_lla[1],self.home_lla[2],latlon_unit='deg',alt_unit='m',model='wgs84')
                    x,y = nearest_square_method(len(self.formation_sync_list),self.local_id,20,20)
                    self.cnt.updateSp(x+self.offset_xyz[1],y+self.offset_xyz[0],10)
                    ...
                    self.reach_position(x+self.offset_xyz[1],y+self.offset_xyz[0],10,position[0][3],30)
                    self.reach_position(x+self.offset_xyz[1],y+self.offset_xyz[0],position[0][2],position[0][3],30)
                    self.formation_sync=1
            self.formation_sync_list[self.local_id-1]=self.formation_sync        
            while True:
                rospy.loginfo('Hold position till sync in formation')
                ...
                if self.formation_sync_func(1)==True:
                    break
                rate.sleep()
            self.waypoint=[[0,60],[-60,60],[-60,0],[0,0]]
            for i in range(len(self.waypoint)):
                self.reach_position(x+self.offset_xyz[1]+self.waypoint[i][0],y+self.offset_xyz[0]+self.waypoint[i][1],position[0][2],position[0][3]*(i+1),30)
                self.formation_sync=i+2
                self.formation_sync_list[self.local_id-1]=self.formation_sync
                while True:
                    rospy.loginfo('Hold position till sync in formationif self.formation_sync_func(i+2)==True:')
                    rospy.loginfo('{}'.format(self.formation_sync_list))
                    self.reach_position(x+self.offset_xyz[1]+self.waypoint[i][0],y+self.offset_xyz[0]+self.waypoint[i][1],position[0][2],position[0][3]*(i+1),30)                
                    if self.formation_sync_func(i+2)==True:
                        self.cnt.set_hdg(position[0][3]*(i+2))
                        for _ in range(3):
                            time.sleep(0.9)
                        break
                    rate.sleep()
            while True:            
                if self.local_id==1 or self.formation_sync_list[self.local_id-2]==100:
                    self.reach_position(x+self.offset_xyz[1]+self.waypoint[i][0],y+self.offset_xyz[0]+self.waypoint[i][1],10,position[0][3]*(i+1),30)
                    break
